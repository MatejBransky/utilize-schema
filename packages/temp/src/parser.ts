...
export function parse(
  ...
  // TODO: edge case, jeste jsem nevyuzil
  if (isPrimitive(schema)) {
    if (isBoolean(schema)) return parseBooleanSchema(schema, keyName, options);
    ...
    case "UNTYPED_ARRAY": {
      ...
      if (minItems > 0 || maxItems >= 0) {
        return {
          ...
          params: new Array(Math.max(maxItems, minItems) || 0).fill(params),
...
function standaloneName(
  ...
  if (name) return generateName(name, usedNames);
}
...
function parseSuperTypes(
  ...
  return superTypes.map(
    (_) =>
      parse(_, options, undefined, processed, usedNames) as TNamedInterface,
  );
}
...
function parseSchema(
  ...
  switch (schema.additionalProperties) {
    case undefined:
    case true:
      if (singlePatternProperty) return asts;
