	...
	switch (ast.type) {
		...
		case 'UNION':
			return [
				...
				.join('\n');
		default:
			if (hasStandaloneName(ast)) return generateStandaloneType(ast, options);
...
export const generateType = memoize(
	generateTypeUnmemoized
) as typeof generateTypeUnmemoized;

function generateRawType(ast: AST, options: Options): string {
	if (hasStandaloneName(ast)) return toSafeIdentifier(ast.standaloneName);
...
function generateInterface(ast: TInterface, options: Options): string {
	const params = ast.params.filter(
		(_) => !_.isPatternProperty && !_.isUnreachableDefinition
	);
	...
	return [`{`, ...lines.map((_) => `  ${_}`), `}`].join('\n');
}

function generateComment(comment?: string, deprecated?: boolean): string {
	const commentLines = ['/**'];
	if (deprecated) commentLines.push(' * @deprecated');
...
function escapeKeyName(keyName: string): string {
	if (
		keyName.length &&
		/[A-Z_$]/i.test(keyName.charAt(0)) &&
		/^[\w$]+$/.test(keyName)
	)
		return keyName;

	if (keyName === '[k: string]') return keyName;
