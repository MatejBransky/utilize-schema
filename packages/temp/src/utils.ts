...
export function traverse(
  ...
  if (processed.has(schema)) return;

  processed.add(schema);
  callback(schema, key ?? null);

  if (schema.anyOf) traverseArray(schema.anyOf, callback, processed);

  if (schema.allOf) traverseArray(schema.allOf, callback, processed);

  if (schema.oneOf) traverseArray(schema.oneOf, callback, processed);
  ...
  if (schema.items) {
    const { items } = schema;
    if (Array.isArray(items)) traverseArray(items, callback, processed);
    else traverse(items, callback, processed);
  }
  ...
  if (schema.$defs) traverseObjectKeys(schema.$defs, callback, processed);

  if (schema.not) traverse(schema.not, callback, processed);
...
export function toSafeIdentifier(string: string) {
  if (isSafeIdentifier(string)) return string;
...
export function normalizeIdentifier(string: string) {
  return upperFirst(
    ...
    deburr(string)
      ...
      .replace(/([\d$]+[a-z])/gi, (match) => match.toUpperCase())
      // uppercase first letter after whitespace
      .replace(/\s+([a-z])/gi, (match) => trim(match.toUpperCase()))
...
export function generateName(from: string, usedNames: Set<string>) {
  let name: string = normalizeIdentifier(from);
  if (!name) name = "NoName";
...
export function maybeStripDefault(schema: LinkedJSONSchema): LinkedJSONSchema {
  ...
  switch (schema.type) {
    case "array":
      if (Array.isArray(schema.default)) return schema;

      break;
    case "boolean":
      if (typeof schema.default === "boolean") return schema;

      break;
    case "integer":
    case "number":
      if (typeof schema.default === "number") return schema;

      break;
    case "string":
      if (typeof schema.default === "string") return schema;
    ...
    case "null":
      ...
      break;
    case "object":
      if (isPlainObject(schema.default)) return schema;
...
export function isSchemaLike(schema: LinkedJSONSchema) {
  ...
  if (JSON_SCHEMA_KEYWORDS.some((_) => parent[_] === schema)) return false;
